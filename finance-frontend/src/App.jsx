...
const App = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const recordsRes = await fetch(`${baseUrl}/api/records`, config);

      if (recordsRes.status === 403) {
        showToast("Session Expired: Access Revoked", "error");
        handleLogout();
        return;
      }
      ...
      if (recordsRes.ok) {
        ...
        setRecords(recordsData.sort((a, b) => new Date(b.date || b.created_at) - new Date(a.date || a.created_at)));
      }

      const summaryRes = await fetch(`${baseUrl}/api/records/summary`, config);
      if (summaryRes.ok) {
      ...
      showToast("Data Sync Failed: Check connection", "error");
    }
  ...
  const handleDeleteRequest = (id, type = 'record') => {
    if (user.role === 'ROLE_VIEWER') return showToast("Permission Denied: Viewers cannot delete data", "error");
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-900 antialiased overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-3 group/profile">
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="overflow-hidden">
                      <p className="text-[11px] font-black leading-none truncate w-24">{user.name}</p>
                      <p className="text-[9px] text-slate-500 mt-1 uppercase font-black tracking-tighter italic">{user.role?.replace('ROLE_', '')}</p>
                    </div>
                  </div>
                  <button
              ...
              <div className="flex gap-4 items-center">
                {activeTab === 'Dashboard' && (
                  <div className={`relative flex items-center transition-all duration-500 ease-out ${searchTerm.length > 0 ? 'scale-105 -translate-x-8' : ''}`}>
                    ...
                    />
                    {searchTerm.length > 0 && (
                      <button onClick={() => setSearchTerm('')} className="absolute right-3 bg-slate-100 hover:bg-rose-100 hover:text-rose-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 transition-all border-none cursor-pointer flex items-center gap-2 group">
                        <span>Clear</span>
                ...
                {searchTerm.length === 0 && (
                  <div className="relative animate-in fade-in duration-300">
                    ...
                    >
                      <div className="p-1.5 bg-indigo-50 rounded-lg"><Shield size={14} className="text-indigo-600" /></div>
                      ...
                      {user.role === 'ROLE_ADMIN' && <ChevronDown size={14} className={`text-slate-400 transition-transform ${isRoleOpen ? 'rotate-180' : ''}`} />}
                    </button>
            ...
            <div className="grid grid-cols-12 gap-8 relative">
                ...
                <>
                    ...
                    <div className={`bg-white rounded-[40px] transition-all duration-500 shadow-sm border border-slate-100 ${searchTerm.length > 0 ? 'shadow-2xl ring-2 ring-indigo-500/10 overflow-hidden' : ''}`}>
                        ...
                        <div className="px-10 py-8 border-b border-slate-50 flex items-center justify-between bg-gradient-to-r from-white to-indigo-50/30 animate-in slide-in-from-top-4 duration-500">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em]">Scanning records • Found {filteredRecords.length} results</p>
                            </div>
                          </div>
                        </div>
                      )}
                      {searchTerm.length > 0 && filteredRecords.length === 0 ? (
                        <div className="p-32 flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-500">
                          ...
                          <button onClick={() => setSearchTerm('')} className="mt-8 px-6 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-lg active:scale-95">
                            Clear Global Search
                          </button>
                        </div>
                      ) : (
                  ...
                  {searchTerm.length === 0 && (
                    <div className="col-span-12 lg:col-span-4 animate-in fade-in duration-500">
                      <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 sticky top-8 transition-all hover:shadow-xl hover:border-indigo-100">
              ...
              {activeTab === 'Analytics' && (
                <div className="col-span-12 bg-white p-10 rounded-[32px] border border-slate-100 shadow-sm animate-in fade-in duration-500">
                  ...
                  <div className="h-96">
                    {(() => {
                      const last7 = [...records].sort((a, b) => new Date(a.date) - new Date(b.date)).slice(-7);
                      return (
                        <Line
                          data={{
              ...
              {activeTab === 'Wallet' && (
                <div className="col-span-12 md:col-span-5 bg-slate-900 p-10 rounded-[40px] relative overflow-hidden shadow-2xl text-white animate-in zoom-in-95 duration-500">
                  ...
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-10 italic">{user.role.replace('ROLE_', '')} ACCOUNT</p>
                  <h2 className="text-5xl font-black mb-10 tracking-tighter italic">₹{Number(summary.netBalance).toLocaleString()}</h2>
                  <div>
              ...
              {(activeTab === 'Settings' || activeTab === 'Provision') && user.role === 'ROLE_ADMIN' && (
                <div className="col-span-12 animate-in fade-in duration-500">
                  <UserManagement
                    ...
                    onDeleteUser={async (id) => handleDeleteRequest(id, 'user')}
      ...
      {notification && (
        <div className="fixed top-10 right-10 z-[1000] flex items-center p-5 bg-white rounded-[32px] shadow-2xl border border-slate-100 min-w-[320px] animate-in slide-in-from-right-full fade-in duration-500 ease-out overflow-hidden">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-lg ${notification.type === 'success' ? 'bg-emerald-500 text-white animate-bounce' : notification.type === 'error' ? 'bg-rose-500 text-white animate-pulse' : 'bg-orange-500 text-white animate-pulse'}`}>
          ...
          <div className="flex-grow ml-4 text-left">
            <h4 className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 ${notification.type === 'success' ? 'text-emerald-600' : notification.type === 'error' ? 'text-rose-600' : 'text-orange-600'}`}>{notification.type === 'success' ? 'Success Verified' : 'System Error'}</h4>
            <p className="text-sm font-bold text-slate-800 tracking-tight leading-tight">{notification.message}</p>
          </div>
          <button onClick={() => setNotification(null)} className="ml-4 p-2 hover:bg-slate-100 rounded-xl transition-all text-slate-400 hover:text-slate-900 border-none cursor-pointer bg-transparent outline-none"><X size={18} /></button>
          <div className="absolute bottom-0 left-0 h-1.5 w-full bg-slate-100">
            <div className={`h-full transition-all ease-linear ${notification.type === 'success' ? 'bg-emerald-500' : notification.type === 'error' ? 'bg-rose-500' : 'bg-orange-500'}`} style={{ animation: 'shrink 3s linear forwards' }} />
          </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[300] flex items-center justify-center p-4">
          <div className="bg-white rounded-[40px] w-full max-w-sm shadow-2xl overflow-hidden p-10 text-center animate-in zoom-in-95 duration-200">
            <div className="w-24 h-24 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner ring-4 ring-rose-100/50"><XCircle size={48} className="animate-in zoom-in duration-300" /></div>
            ...
            <div className="flex gap-4">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-5 bg-slate-50 text-slate-500 rounded-[24px] font-black uppercase tracking-widest text-[10px] hover:bg-slate-100 transition-all border border-slate-100">Cancel</button>
              <button
                onClick={async () => {
                  setLoading(true);
                ...
                disabled={loading}
                className="flex-1 py-5 bg-rose-500 text-white rounded-[24px] font-black uppercase tracking-widest text-[10px] hover:bg-rose-600 shadow-xl shadow-rose-200 transition-all border-none flex items-center justify-center gap-2"
              >
